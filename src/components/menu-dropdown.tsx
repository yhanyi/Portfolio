        ...
        <div className="md:hidden flex items-center justify-center">
            <motion.div animate={open ? "open" : "closed"} className="relative">
                ...
                onClick={() => setOpen((pv) => !pv)}>
                    <div>
                        <span className={`bg-dark dark:bg-light transition-all duration-300 block h-0.5 w-4 rounded-sm ${open ? `rotate-45 translate-y-1` : `-translate-y-0.5`}`}></span>
                        <span className={`bg-dark dark:bg-light transition-all duration-300 block h-0.5 w-4 rounded-sm my-0.5 ${open ? `opacity-0` : `opacity-100`}`}></span>
                        <span className={`bg-dark dark:bg-light transition-all duration-300 block h-0.5 w-4 rounded-sm ${open ? `-rotate-45 -translate-y-1` : `translate-y-0.5`}`}></span>
                    </div>
                </button>
